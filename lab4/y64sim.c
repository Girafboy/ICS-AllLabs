...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if((op == A_ADD && !(argA^argB) && (argB^val)) 
    || (op == A_SUB && (argA^argB) && (argB^val)))
        ovf = TRUE;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        if(!get_long_val(sim->m, get_reg_val(sim->r, GET_REGB(codefun)) + imm, &temp)){
      ...
      case I_ALU: /* 6:x regA:regB */
        ...
        sim->cc = compute_cc(ifun, GET_REGA(codefun), GET_REGB(codefun), temp);
        sim->pc = next_pc;
        break;
      case I_JMP: /* 7:x imm */
        if(cond_doit(sim->cc, ifun))
